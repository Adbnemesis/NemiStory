...
func _ready() -> void:
	...
	# Instantiate Leon in Monochrome
	...
	# Instantiate Edgar in Monochrome
	...
	header.text = "BRAWL STARS — MONOCHROME INK-WASH STORYTELLING MODE"
	header.add_theme_font_size_override("font_size", 28)
	header.add_theme_color_override("font_color", Color(0.12, 0.12, 0.15))
	title_box.add_child(header)
	
	var subheader = Label.new()
	subheader.text = "Zero-Motion Comedic Deadpan Freeze • Hand-Drawn Pen & Ink Storytime Style"
