#!/usr/bin/env python3
"""
generate_edgar.py - Production Dialogue Generator for Edgar (Cutenemi / Brawl Stars)
Model: mlx-community/Qwen3-TTS-12Hz-1.7B-Base-bf16
Voice: Deadpan, cynical, relaxed gamer, flat matter-of-fact delivery
...
CONFIG_PATH = os.path.join(WORKSPACE, "brawl_stars/voices/edgar/config/edgar_config.json")
PRESETS_PATH = os.path.join(WORKSPACE, "brawl_stars/voices/edgar/config/edgar_presets.json")
...
def get_model():
    ...
    if _CACHED_MODEL is None:
        ...
        print(f"[Edgar TTS] Loading model '{model_repo}' on Apple Silicon GPU...")
        _CACHED_MODEL = load_model(model_repo)
        print("[Edgar TTS] Model loaded.")
...
def generate_line(
    ...
    preset_data = PRESETS.get(emotion, PRESETS["deadpan"])
    ...
    print(f"[Edgar TTS] Synthesizing: \"{text}\" (Emotion: {emotion}, Pitch: {pitch_semitones:+.1f} st)")
...
def main():
    parser = argparse.ArgumentParser(description="Generate dialogue for Edgar (Brawl Stars)")
    parser.add_argument("--text", type=str, help="Dialogue line text to speak")
    parser.add_argument("--output", "-o", type=str, default="edgar_output.wav", help="Path to output WAV file")
    parser.add_argument("--emotion", type=str, default="deadpan", choices=list(PRESETS.keys()), help="Emotional acting preset")
    parser.add_argument("--pitch", type=float, default=0.0, help="Pitch shift in semitones (e.g. +1.5 or -2.0)")
    ...
    if args.script:
        ...
        for i, item in enumerate(script_data):
            out_p = item.get("output", f"edgar_line_{i+1:02d}.wav")
            generate_line(
                text=item["text"],
                output_path=out_p,
                emotion=item.get("emotion", "deadpan"),
