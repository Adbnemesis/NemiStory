class_name CosmoInkStroke
extends RefCounted

## Calligraphic variable-width ink stroke renderer for the Brawl Stars Cosmo character system.
## Produces organic hand-drawn linework with pen-pressure tapering, preventing sterile uniform vectors.
...
var stroke_color: Color = Color("#201c24")

func _init(p_points: PackedVector2Array = PackedVector2Array(), p_width: float = 3.2, p_profile: Profile = Profile.TAPER_BOTH, p_color: Color = Color("#201c24")) -> void:
	points = p_points
	base_width = p_width
	profile = p_profile
	stroke_color = p_color

static func from_curve(curve: Curve2D, width: float = 3.2, prof: int = Profile.TAPER_BOTH, col: Color = Color("#201c24"), max_stages: int = 5, tolerance_deg: float = 2.5) -> RefCounted:
	var dense_pts: PackedVector2Array = curve.tessellate(max_stages, tolerance_deg)
	var stroke_script = load("res://brawl_stars/scripts/CosmoInkStroke.gd")
	return stroke_script.new(dense_pts, width, prof, col)

static func from_points(pts: PackedVector2Array, width: float = 3.2, prof: int = Profile.TAPER_BOTH, col: Color = Color("#201c24")) -> RefCounted:
	var stroke_script = load("res://brawl_stars/scripts/CosmoInkStroke.gd")
