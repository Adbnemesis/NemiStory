...
## Conversational gesture: Measuring tiny scale with both hands framing in front of chest
func gesture_tiny_scale(duration: float = 0.22) -> void:
	if not left_upper_arm or not right_upper_arm or not left_lower_arm or not right_lower_arm:
		return
	if left_hand_visual: left_hand_visual.set("hand_pose", NemiLimbPart.HandPose.OPEN)
	if right_hand_visual: right_hand_visual.set("hand_pose", NemiLimbPart.HandPose.OPEN)
	var tw: Tween = character.create_tween().set_parallel(true)
	tw.tween_property(left_upper_arm, "rotation", deg_to_rad(-24.0), duration).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	tw.tween_property(left_lower_arm, "rotation", deg_to_rad(-62.0), duration).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	tw.tween_property(right_upper_arm, "rotation", deg_to_rad(24.0), duration).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	tw.tween_property(right_lower_arm, "rotation", deg_to_rad(62.0), duration).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	await tw.finished
