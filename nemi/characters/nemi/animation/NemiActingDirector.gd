...
const NemiLipSync = preload("res://nemi/characters/nemi/animation/actions/NemiLipSync.gd")
...
func stop_speech(rest_shape: String = "rest") -> void:
	if lipsync:
		lipsync.stop_speech(rest_shape)
...
func gesture_tiny_scale(duration: float = 0.22) -> void:
	var token: int = request_action("GESTURE_TINY_SCALE", NemiTiming.Priority.MINOR_GESTURE, 0.5, duration)
	await body.gesture_tiny_scale(duration)
	finish_action(token)
