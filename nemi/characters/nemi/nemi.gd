...
const NemiProportions = preload("res://nemi/characters/nemi/NemiProportions.gd")
const NemiRigDebug = preload("res://nemi/characters/nemi/drawing/NemiRigDebug.gd")
const NemiActingDirector = preload("res://nemi/characters/nemi/animation/NemiActingDirector.gd")
const NemiFXDirector = preload("res://nemi/characters/nemi/fx/NemiFXDirector.gd")
...
func recoil(strength_or_offset = 0.4, _duration: float = 0.12) -> void:
	set_pose("recoiling", 0.08)
	if strength_or_offset is float or strength_or_offset is int:
		shake(float(strength_or_offset))
	elif strength_or_offset is Vector2:
		shake(clampf(absf(strength_or_offset.x) / 50.0, 0.2, 1.0))
...
## Stops speech and returns mouth to rest shape
func stop_speech(rest_shape: String = "rest") -> void:
	if not actor: _ensure_nodes()
	if actor and actor.has_method("stop_speech"):
		actor.stop_speech(rest_shape)
	elif face and face.has_method("set_mouth_shape"):
...
## Measuring tiny scale with both hands framing in front of chest
func gesture_tiny_scale(duration: float = 0.22) -> void:
	if not actor: _ensure_nodes()
	if actor: await actor.gesture_tiny_scale(duration)
...
## High-level API aliases and helpers for storytelling scripts and showcase
func pose(pose_name: String, transition_time: float = 0.0) -> void:
	set_pose(pose_name, transition_time)

func expression(expr_name: String) -> void:
	set_expression(expr_name)

func equip_prop(_prop_name: String, _offset: Vector2 = Vector2.ZERO) -> void:
	pass

func unequip_prop() -> void:
	pass

func start_stepped_bounce(height: float = 16.0, hz: float = 6.0) -> void:
	if actor and actor.has_method("start_stepped_bounce"):
		actor.start_stepped_bounce(height, hz)

func stop_stepped_bounce() -> void:
	if actor and actor.has_method("stop_stepped_bounce"):
		actor.stop_stepped_bounce()

func eyebrow_raise(hold_duration: float = 0.45) -> void:
	if face and face.has_method("set_brow_expression"):
		face.set_brow_expression("surprised")
		await get_tree().create_timer(hold_duration).timeout
		face.set_brow_expression("neutral")

func reset_lean(duration: float = 0.1) -> void:
	lean(0.0, duration)
