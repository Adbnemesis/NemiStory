...
const PRESET_ZOOMS = {
	ShotPreset.WIDE: 1.0,
	ShotPreset.MEDIUM: 1.15,
	ShotPreset.MEDIUM_CLOSEUP: 1.45,
	ShotPreset.CLOSEUP: 1.95,
	ShotPreset.EXTREME_CLOSEUP: 2.5
}

const PRESET_OFFSETS = {
	ShotPreset.WIDE: Vector2(0, 0),
	ShotPreset.MEDIUM: Vector2(0, -40),
	ShotPreset.MEDIUM_CLOSEUP: Vector2(0, -110),
	ShotPreset.CLOSEUP: Vector2(0, -150),
	ShotPreset.EXTREME_CLOSEUP: Vector2(0, -180)
}
...
func apply_preset(preset: ShotPreset, duration: float = 0.35, target_node: Node2D = null) -> Signal:
	...
	var dest_pos: Vector2 = _base_offset + PRESET_OFFSETS.get(preset, Vector2.ZERO)
	
	if is_instance_valid(target_node):
		dest_pos = target_node.global_position + PRESET_OFFSETS.get(preset, Vector2.ZERO)
