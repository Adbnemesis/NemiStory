...
func _draw() -> void:
	...
	match doodle_type:
		...
		Type.MINI_HOUSE: _draw_mini_house(ink, w)
		Type.MINI_CAT_HEAD: _draw_mini_cat_head(ink, w)
		Type.MINI_PAW: _draw_mini_paw(ink, w)
		Type.MINI_MILK_BOTTLE: _draw_mini_milk_bottle(ink, w)
...
func _draw_mini_house(ink: Color, width: float) -> void:
	var sz := doodle_size * 0.95
	var p := clampf(draw_progress, 0.0, 1.0)
	
	# Phase 1: Base Walls (0.0 to 0.4)
	if p > 0.01:
		var p1 := clampf(p / 0.4, 0.0, 1.0)
		var wall_pts := PackedVector2Array([
			Vector2(-sz * 0.55, sz * 0.55),
			Vector2(-sz * 0.55, -sz * 0.05),
			Vector2(sz * 0.55, -sz * 0.05),
			Vector2(sz * 0.55, sz * 0.55),
			Vector2(-sz * 0.55, sz * 0.55)
		])
		var max_idx := int(float(wall_pts.size() - 1) * p1) + 1
		var pts_slice := PackedVector2Array()
		for i in range(mini(max_idx, wall_pts.size())):
			pts_slice.append(wall_pts[i])
		if pts_slice.size() >= 2:
			InkStroke.from_points(pts_slice, width * 1.1, InkStroke.Profile.UNIFORM, ink).draw_to(self)
			
	# Phase 2: Pitched Roof & Chimney (0.35 to 0.7)
	if p > 0.35:
		var p2 := clampf((p - 0.35) / 0.35, 0.0, 1.0)
		var roof_pts := PackedVector2Array([
			Vector2(-sz * 0.7, 0.0),
			Vector2(0.0, -sz * 0.65),
			Vector2(sz * 0.7, 0.0)
		])
		var pts_roof := PackedVector2Array()
		var roof_count := int(float(roof_pts.size() - 1) * p2) + 1
		for i in range(mini(roof_count, roof_pts.size())):
			pts_roof.append(roof_pts[i])
		if pts_roof.size() >= 2:
			InkStroke.from_points(pts_roof, width * 1.25, InkStroke.Profile.TAPER_BOTH, ink).draw_to(self)
			
		# Chimney
		if p2 > 0.7:
			var ch := PackedVector2Array([
				Vector2(sz * 0.32, -sz * 0.35),
				Vector2(sz * 0.32, -sz * 0.62),
				Vector2(sz * 0.48, -sz * 0.62),
				Vector2(sz * 0.48, -sz * 0.22)
			])
			InkStroke.from_points(ch, width * 0.9, InkStroke.Profile.UNIFORM, ink).draw_to(self)
			
	# Phase 3: Door & Window (0.7 to 1.0)
	if p > 0.7:
		var p3 := clampf((p - 0.7) / 0.3, 0.0, 1.0)
		# Arched / Square Door
		var door := PackedVector2Array([
			Vector2(-sz * 0.16, sz * 0.55),
			Vector2(-sz * 0.16, sz * 0.12),
			Vector2(sz * 0.16, sz * 0.12),
			Vector2(sz * 0.16, sz * 0.55)
		])
		InkStroke.from_points(door, width * 0.9, InkStroke.Profile.UNIFORM, ink).draw_to(self)
		# Door handle
		draw_circle(Vector2(sz * 0.10, sz * 0.35), 1.6, ink)
		
		# Window
		if p3 > 0.5:
			var win_rect := Rect2(-sz * 0.42, sz * 0.08, sz * 0.22, sz * 0.22)
			draw_rect(win_rect, ink, false, width * 0.8)
			# Window cross
			draw_line(Vector2(win_rect.position.x + win_rect.size.x * 0.5, win_rect.position.y), Vector2(win_rect.position.x + win_rect.size.x * 0.5, win_rect.end.y), ink, width * 0.6)
			draw_line(Vector2(win_rect.position.x, win_rect.position.y + win_rect.size.y * 0.5), Vector2(win_rect.end.x, win_rect.position.y + win_rect.size.y * 0.5), ink, width * 0.6)

func _draw_mini_cat_head(ink: Color, width: float) -> void:
	var sz := doodle_size * 0.85
	var p := clampf(draw_progress, 0.0, 1.0)
	
	# Head outline with pointy ears
	var head_pts := PackedVector2Array([
		Vector2(-sz * 0.6, sz * 0.1),
		Vector2(-sz * 0.5, -sz * 0.2),
		Vector2(-sz * 0.65, -sz * 0.75), # Left ear tip
		Vector2(-sz * 0.2, -sz * 0.45), # Left ear base
		Vector2(sz * 0.2, -sz * 0.45),  # Right ear base
		Vector2(sz * 0.65, -sz * 0.75),  # Right ear tip
		Vector2(sz * 0.5, -sz * 0.2),
		Vector2(sz * 0.6, sz * 0.1),
		Vector2(sz * 0.4, sz * 0.5),
		Vector2(-sz * 0.4, sz * 0.5),
		Vector2(-sz * 0.6, sz * 0.1)
	])
	var count := int(float(head_pts.size() - 1) * clampf(p / 0.6, 0.0, 1.0)) + 1
	var slice := PackedVector2Array()
	for i in range(mini(count, head_pts.size())):
		slice.append(head_pts[i])
	if slice.size() >= 2:
		InkStroke.from_points(slice, width * 1.15, InkStroke.Profile.UNIFORM, ink).draw_to(self)
		
	# Happy eyes and nose (0.5 to 0.8)
	if p > 0.5:
		var p_face := clampf((p - 0.5) / 0.3, 0.0, 1.0)
		if p_face > 0.3:
			# Left eye arc ^
			var l_eye := PackedVector2Array([Vector2(-sz * 0.35, -sz * 0.02), Vector2(-sz * 0.25, -sz * 0.12), Vector2(-sz * 0.15, -sz * 0.02)])
			InkStroke.from_points(l_eye, width * 1.1, InkStroke.Profile.TAPER_BOTH, ink).draw_to(self)
			# Right eye arc ^
			var r_eye := PackedVector2Array([Vector2(sz * 0.15, -sz * 0.02), Vector2(sz * 0.25, -sz * 0.12), Vector2(sz * 0.35, -sz * 0.02)])
			InkStroke.from_points(r_eye, width * 1.1, InkStroke.Profile.TAPER_BOTH, ink).draw_to(self)
			# Nose & mouth
			draw_circle(Vector2(0.0, sz * 0.12), 1.8, ink)
			var mouth_l := PackedVector2Array([Vector2(0.0, sz * 0.14), Vector2(-sz * 0.12, sz * 0.24)])
			var mouth_r := PackedVector2Array([Vector2(0.0, sz * 0.14), Vector2(sz * 0.12, sz * 0.24)])
			InkStroke.from_points(mouth_l, width * 0.8, InkStroke.Profile.TAPER_BOTH, ink).draw_to(self)
			InkStroke.from_points(mouth_r, width * 0.8, InkStroke.Profile.TAPER_BOTH, ink).draw_to(self)
			
	# Whiskers (0.75 to 1.0)
	if p > 0.75:
		var pw := clampf((p - 0.75) / 0.25, 0.0, 1.0)
		# Left whiskers
		draw_line(Vector2(-sz * 0.3, sz * 0.08), Vector2(-sz * 0.3 - sz * 0.45 * pw, sz * 0.02), ink, width * 0.7)
		draw_line(Vector2(-sz * 0.3, sz * 0.18), Vector2(-sz * 0.3 - sz * 0.45 * pw, sz * 0.24), ink, width * 0.7)
		# Right whiskers
		draw_line(Vector2(sz * 0.3, sz * 0.08), Vector2(sz * 0.3 + sz * 0.45 * pw, sz * 0.02), ink, width * 0.7)
		draw_line(Vector2(sz * 0.3, sz * 0.18), Vector2(sz * 0.3 + sz * 0.45 * pw, sz * 0.24), ink, width * 0.7)

func _draw_mini_paw(ink: Color, width: float) -> void:
	var sz := doodle_size * 0.75
	var p := clampf(draw_progress, 0.0, 1.0)
	
	# Main palm pad
	if p > 0.1:
		var p_pad := clampf((p - 0.1) / 0.5, 0.0, 1.0)
		var pad_pts := PackedVector2Array()
		for i in range(16):
			var a := float(i) * TAU / 15.0
			pad_pts.append(Vector2(cos(a) * sz * 0.38 * p_pad, sin(a) * sz * 0.30 * p_pad + sz * 0.15))
		if pad_pts.size() >= 3:
			draw_colored_polygon(pad_pts, ink)
			
	# Three toe pads
	if p > 0.5:
		var p_toe := clampf((p - 0.5) / 0.5, 0.0, 1.0)
		var toes := [
			Vector2(-sz * 0.32, -sz * 0.25),
			Vector2(0.0, -sz * 0.38),
			Vector2(sz * 0.32, -sz * 0.25)
		]
		for toe in toes:
			draw_circle(toe, sz * 0.14 * p_toe, ink)

func _draw_mini_milk_bottle(ink: Color, width: float) -> void:
	var sz := doodle_size * 0.9
	var p := clampf(draw_progress, 0.0, 1.0)
	
	# Bottle contour (0.0 to 0.7)
	var bottle_pts := PackedVector2Array([
		Vector2(-sz * 0.35, sz * 0.6),
		Vector2(-sz * 0.35, -sz * 0.15),
		Vector2(-sz * 0.18, -sz * 0.45),
		Vector2(-sz * 0.18, -sz * 0.75), # Cap left
		Vector2(sz * 0.18, -sz * 0.75),  # Cap right
		Vector2(sz * 0.18, -sz * 0.45),
		Vector2(sz * 0.35, -sz * 0.15),
		Vector2(sz * 0.35, sz * 0.6),
		Vector2(-sz * 0.35, sz * 0.6)
	])
	var count := int(float(bottle_pts.size() - 1) * clampf(p / 0.7, 0.0, 1.0)) + 1
	var slice := PackedVector2Array()
	for i in range(mini(count, bottle_pts.size())):
		slice.append(bottle_pts[i])
	if slice.size() >= 2:
		InkStroke.from_points(slice, width * 1.1, InkStroke.Profile.UNIFORM, ink).draw_to(self)
		
	# Cap line & label (0.6 to 1.0)
	if p > 0.6:
		var p_lbl := clampf((p - 0.6) / 0.4, 0.0, 1.0)
		draw_line(Vector2(-sz * 0.18, -sz * 0.65), Vector2(sz * 0.18, -sz * 0.65), ink, width * 0.8)
		# Label box
		if p_lbl > 0.3:
			var lbl_rect := Rect2(-sz * 0.26, -sz * 0.05, sz * 0.52, sz * 0.40)
			draw_rect(lbl_rect, ink, false, width * 0.7)
			# Small milk droplet symbol
			draw_circle(Vector2(0.0, sz * 0.15), 3.0, ink)
