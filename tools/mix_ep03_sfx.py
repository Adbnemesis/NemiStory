#!/usr/bin/env python3
"""
tools/mix_ep03_sfx.py
Master Audio Mixing Pipeline for Nemi Episode 03: "My Mom Scolded Me"

- Reads nemi/episodes/ep03_scolded/ep03_sfx_timeline.json
- Reads common/audio/sfx/sfx_catalog.json
- Loads master voiceover (nemi/episodes/ep03_scolded/audio/EP03_voice.wav)
- Precision mixes 16 SFX cues at exact timestamps with calibrated dB gains
- Asserts dialogue clarity, zero clipping (>=1.0 dBFS headroom), and preserves deadpan silences
- Exports master audio: nemi/episodes/ep03_scolded/audio/EP03_audio_sfx_master.wav
"""
...
TIMELINE_PATH = os.path.join(BASE_DIR, "nemi", "episodes", "ep03_scolded", "ep03_sfx_timeline.json")
CATALOG_PATH = os.path.join(BASE_DIR, "common", "audio", "sfx", "sfx_catalog.json")
VOICE_PATH = os.path.join(BASE_DIR, "nemi", "episodes", "ep03_scolded", "audio", "EP03_voice.wav")
OUTPUT_DIR = os.path.join(BASE_DIR, "nemi", "episodes", "ep03_scolded", "audio")

MASTER_AUDIO_PATH = os.path.join(OUTPUT_DIR, "EP03_audio_sfx_master.wav")
SFX_ONLY_AUDIO_PATH = os.path.join(OUTPUT_DIR, "EP03_audio_sfx_only.wav")
...
def main():
    print("============================================================")
    print("  NEMI EPISODE 03: AUDIO SFX MASTERING & PRECISION MIX")
    print("============================================================")
    ...
    for idx, cue in enumerate(timeline["cues"]):
        ...
        # Enforce the <= 2.0s rule for event sfx
        if cue_dur > 2.5:
            print(f"  [WARN] SFX {sfx_id} duration is {cue_dur:.2f}s (>2.0s). Trimming tail.")
